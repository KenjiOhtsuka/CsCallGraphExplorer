...
| 3 | **CalleesQuery.cs** | Constructor initializer calls (`: this(...)`, `: base(...)`) not recorded in callee graph | Major | **Fixed** in PR #25 — scans `ConstructorInitializerSyntax` and resolves each via `GetSymbolInfo`; `: this` chain and `: base` chain covered by tests |
