...
| 3 | **CalleesQuery.cs** | Constructor initializer calls (`: this(...)`, `: base(...)`) not recorded in callee graph | Major | **Fixed** in PR #25 — scans `ConstructorInitializerSyntax` and `PrimaryConstructorBaseTypeSyntax`, resolves each via `GetSymbolInfo`; `: this` chain, `: base` chain, and C# 12 record primary-constructor chains covered by tests. Known limitations: implicit `base()` (compiler-synthesized, no syntax node) is not recorded; caller-side queries do not yet resolve record primary-constructor chains (`SymbolFinder` does not index `PrimaryConstructorBaseTypeSyntax`); member-initializer expressions in primary constructors are not scanned |
...
| 7 | **LspModels.cs** | Advertises `TextDocumentSync.Full` (1) but server does not consume `didOpen`/`didChange` events; should be `None` (0) | Minor | **Fixed** in PR #22 — server advertises `TextDocumentSync.None` (0) since it resolves from disk and does not implement document sync |
| 8 | **CallGraphEngine.cs** | Cache `Lazy<Task<...>>` factory captures caller's `CancellationToken`; on cancellation/retry, entry not evicted | Minor | **Fixed** in PR #22 — factory loads with `CancellationToken.None`; caller token applied via `WaitAsync`; entry evicted on failure |
